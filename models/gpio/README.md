...
状态唯一见 [registry.yaml](../../registry.yaml) 的 `aixsilicon:esl:gpio:0.1.0`。提供 `register` profile：探索 MMIO32 ABI、完整数据、LT 阻塞事务与参数化服务时间，不宣称厂商寄存器兼容或周期精确性能。

- `systemc/include/gpio/config.hpp` 与 `model.hpp`：公开 Config/Model/端口。
- `systemc/src/model.cpp`：实现；`model.yaml`：可执行交付合同。
- [设计与寄存器](docs/design.md)、[上层集成](docs/integration.md)、[验证范围](docs/verification.md)。
- 库 target `aix::esl::gpio`，安装包 `AixEslGpio`；模型不含 sc_main。
