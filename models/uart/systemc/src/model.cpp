...
struct Model::Impl {
    ...
    Config config;
    aix::esl::Mmio32 mmio;
    aix::esl::BoundedQueue<Frame> tx;
    aix::esl::BoundedQueue<unsigned char> rx;
    sc_core::sc_event wake, changed, became_idle;
    std::uint32_t control=0;
    std::uint64_t epoch=0;
    bool overrun=false;
    bool overrun_time_valid=false;
    sc_core::sc_time overrun_time;
    void overflow(){overrun=true;overrun_time_valid=true;overrun_time=sc_core::sc_time_stamp();}
    ...
    explicit Impl(Config c):config(validated(c)),mmio(config.access_latency),tx(config.tx_depth),rx(config.rx_depth){}
};
...
bool Model::resume(){return !reset.read()&&idle()&&impl_->mmio.resume();}
void Model::handle_reset(){
    auto& p=*impl_;p.mmio.reset(reset.read());
    if(reset.read()){++p.epoch;p.tx.clear();p.rx.clear();p.control=0;p.overrun=false;p.overrun_time_valid=false;p.wake.notify(sc_core::SC_ZERO_TIME);}
...
bool Model::receive(unsigned char byte){
    ...
    if(!p.rx.try_push(byte)){p.overflow();p.changed.notify(sc_core::SC_ZERO_TIME);return false;}
    p.changed.notify(sc_core::SC_ZERO_TIME);return true;
}
void Model::run(){
    auto& p=*impl_;
    while(true){
    ...
    impl_->mmio.access(tx,delay,[&](std::uint64_t address,bool write,std::uint32_t& value){
        ...
        if(write){switch(address){
            case reg::data:
                ...
                if(!p.tx.try_push({static_cast<unsigned char>(value),bool(p.control&reg::loopback)}))return tlm::TLM_GENERIC_ERROR_RESPONSE;
            ...
            case reg::error:
                ...
                if((value&1)&&!(p.overrun_time_valid&&p.overrun_time==sc_core::sc_time_stamp()))p.overrun=false;break;
            ...
            case reg::data:
                ...
                value=p.rx.front();p.rx.pop();break;
            case reg::status:value=(!p.rx.empty()?1u:0u)|(p.tx.full()?2u:0u)|(p.tx.empty()?4u:0u)|(p.overrun?8u:0u);break;
