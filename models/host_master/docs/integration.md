...
公开头文件 `<host_master/model.hpp>` 与 `<host_master/config.hpp>`；命名空间 `aix::esl::host_master`。构造 `Model(sc_module_name, Config)`；每实例独立状态，顶层拥有模型和 Config。必需端口见 [manifest](../model.yaml)，漏绑定由 SystemC elaboration 报错。

```cmake
# 源码树：ESL_REPO 指向资产仓库
add_subdirectory("${ESL_REPO}/models/host_master" "models/host_master")
target_link_libraries(my_system PRIVATE aix::esl::host_master)
# 安装包：用以下两行替换 add_subdirectory/链接行
find_package(AixEslHostMaster 0.1 CONFIG REQUIRED)
target_link_libraries(my_system PRIVATE aix::esl::host_master)
```
