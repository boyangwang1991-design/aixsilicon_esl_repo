...
> 一次真实的 NPU SRAM 架构探索，看 AI 如何改变芯片架构决策
...
为了讲清这件事，我们为 NPU 的共享 SRAM 控制器建了一个 SystemC 性能模型，跑了 1045 个配置点。探索的过程和结论，就是这篇文章要讲的内容：AI 是怎么让这套探索变得可行、可信、可扩展的，以及它最终如何改变了我们对"下一步该往哪走"的判断。
...
这次探索最值得讲的部分，其实不在"哪个方案赢了"，而在于它带出的一个问题：**为什么一个看似合理的优化，在系统层面没有变成收益？**

为了回答这个问题，我们把同一个保留集 GEMM 从三个层面拆开看。
...
> **RTL 阶段，AI 帮你更快地回答"怎么做"；ESL 阶段，AI 帮你更早地回答"做什么"。**
